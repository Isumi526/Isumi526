...
| 領域 | 内容 |
|---|---|
| 公共・自治体 | 相談プラットフォームの新規構築。**150テナント超・エンドユーザー約8万名・累計メッセージ110万件超**の規模で稼働中。全1,700コミット中700件超を担当し、**貢献者14名中1位** |
| ヘルスケア | 従業員1,400名規模の企業の新規事業に約2年参画。PG・デザイナーを含む**4名体制の技術リード** |
| EC | オンラインくじ形式の EC の立ち上げに**初回コミッター**として参画。**累計20万件超の購入処理**を扱う。決済・在庫・配送まで |
| 保険 | 営業担当者向け CRM。**利用者約120名**、全コミットの**約80%を担当**。Flutter + Nuxt 3 + Cloud Functions の3層構成 |
| 建設・内装 | 施工管理 SaaS（sido）。社内利用者34名。要件定義から現場定着まで |
| 冠婚葬祭 | 業務システムの長期保守・機能追加。**800コミット超 / 2年10ヶ月** |
...
```
2013.05 ─ 2016.09   アパレル販売（約3年5ヶ月）
2016.10 ─ 2020.10   投資用不動産の営業（約4年）
2020    ─ 2021      独学でエンジニアへ転向
2021.05 ─ 2026.03   受託開発会社と業務委託契約。30件以上のプロジェクトに参画
2026.04 ─ 現在      直請け + 自社プロダクト開発
```
